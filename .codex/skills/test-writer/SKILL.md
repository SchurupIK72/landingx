...
recommended_next_skill: gui-testing  # Рекомендуемый следующий skill для Codex
```
...
Запустить **explorer sub-agent** (`explorer`) для каждого нового модуля из spec:
...
|----------|--------------|
| Тесты написаны и GREEN | Следующим сообщением вызвать `gui-testing`, например: "протестируй UI" |
